...
const char viewCompletionHandlerKey;
...
-(void)showWithCompletionHandler:(void (^)(NSInteger buttonIndex))completionHandler
{
    ...
    if(completionHandler != nil)
    {
        ...
        objc_setAssociatedObject(self, &viewCompletionHandlerKey, completionHandler, OBJC_ASSOCIATION_COPY);
        alert.delegate = self;
    }
    [alert show];
}
...
{
    UIAlertView *alert = (UIAlertView *)self;
    void (^theCompletionHandler)(NSInteger buttonIndex) = objc_getAssociatedObject(self, &viewCompletionHandlerKey);
                     ...
                     onCancel:(YSCancelBlock)cancelled
{
    ...
    [presentVC presentModalViewController:self animated:NO];
}
