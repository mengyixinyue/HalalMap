...
@property (nonatomic, strong) UIBarButtonItem *confirmButtonItem;
...
-(void)backgroundPressed:(UITapGestureRecognizer *)tap
{
    CGPoint touchPoint = [tap locationInView:self];
    if (touchPoint.y > _pickerTopBarView.frame.origin.y - 10) {
        [MMPickerView dismissWithCompletion:nil];
    }
...
-(void)initializePickerViewInView: (UIView *)view
  ...
  _pickerContainerView = [[UIView alloc] initWithFrame:CGRectMake(0.0, _pickerViewContainerView.bounds.size.height - 260.0, view.width, 260.0)];
  ...
  if (toolbarImage!=nil) {
    ...
    _confirmButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"确定" style:UIBarButtonItemStyleDone target:self action:@selector(dismiss)];
...
//  _pickerViewToolBar.items = @[flexibleSpace, _pickerViewBarButtonItem];
    _pickerViewToolBar.items = [NSArray arrayWithObjects:_cancelButtonItem,spaceButtonItem, _confirmButtonItem, nil];
...
//  [_pickerViewBarButtonItem setTintColor:buttonTextColor];
  ...
  //Add pickerView
  _pickerView = [[UIPickerView alloc] initWithFrame:CGRectMake(0.0, 44.0, view.width, 216.0)];
  ...
  [_pickerView selectRow:selectedRow inComponent:0 animated:YES];
    ...
    UITapGestureRecognizer * tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(backgroundPressed:)];
