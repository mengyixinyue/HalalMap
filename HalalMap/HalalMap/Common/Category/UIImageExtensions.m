...
- (NSData *)compressBelowKNum:(CGFloat)kNum
{
    UIImage *image = self;
    image = [image imageFitInSize:CGSizeMake(1080, 1920)];
    float factor = 1.0;
    NSData *finalData = UIImageJPEGRepresentation(image, factor);
    NSLog(@"origData.length:%fk",(UIImageJPEGRepresentation(image, factor).length)/1024.0);
    while (finalData.length/1024.0 > kNum) {
        if (factor > 0.1) {
            factor -= 0.1;
        }
        else
        ...
        finalData = UIImageJPEGRepresentation(image, factor);
    }
    return finalData;
}
